...
def image_transforms(img): #, stats_fn, id):
    ...
    #img = (img - mean) / std
    img = np.rollaxis(img, 2, 0).astype(np.float32)[:3]
...
def load_dataset(opts, kwargs=None):
    ...
    trn = StreamingGeospatialDataset(train_img_fns, stats_fn, train_label_fns, opts.label_channels, chip_size=opts.chip_size, num_chips_per_tile=20, image_transform=img_transforms, verbose=False)

    val_img_fns = get_imagery_fns(opts.data_dir, "val", opts.dataset)
    val_label_fns = get_labels_fns(opts.data_dir , "val", opts.dataset)
    val_img_fns.sort()
    val_label_fns.sort()
    val = StreamingGeospatialDataset(val_img_fns, stats_fn, val_label_fns, opts.label_channels, chip_size=opts.chip_size, num_chips_per_tile=10, image_transform=img_transforms, verbose=False)
