...
def train(framework, loaders, opts):
    if opts.checkpoint_file == "none":
        ...
        # determine backup folder path and create it if necessary (In here we will keep the best model so far)
        ...
        # load checkpoint and corresponding opts file overwriting command-line args
    ...
    tic = time()
    val_history = {'loss': [], 'val_metric': []}
    train_history = {'loss': []}

    scheduler = lr_scheduler.ReduceLROnPlateau(framework.optimizer, 'max', patience=opts.scheduler_patience)

    stats = {
        'train_losses': [], 'train_losses_epochs': [],
        'val_losses': [], 'val_metric_values': [], 'val_metric_epochs': [],
        'best_val_metric_value': sys.maxsize, 'best_val_epoch': 0.,
        'resume_epochs': []
    }
    ...
    if resume_run:
        ...
        stats['val_metric_values'] = checkpoint['val_metric_values']
        stats['val_metric_epochs'] = checkpoint['val_metric_epochs']
        stats['best_val_metric_value'] = checkpoint['best_val_metric_value']
        ...
        for phase in ['train', 'val']:
            ...
            for n_iter, (X, y) in enumerate(loaders[phase]):
                ...
                if phase == 'val':
                    ...
                    val_metric_value += batch_meanIoU
            ...
            if phase == 'train':
                ...
                if n_iter > 0:
                    val_metric_value /= (n_iter + 1)
                val_history['loss'].append(epoch_loss)
                stats['val_metric_values'].append(val_metric_value)
                stats['val_losses'].append(epoch_loss)
                stats['val_metric_epochs'].append(i)

                scheduler.step(epoch_loss)
                text = "\nVal MAE: {}".format(str(val_metric_value))
                my_log = open(log_file, 'a+')
                my_log.write(text)
                if opts.verbose:
                    print(text)
                is_best = False
                if val_metric_value > stats['best_val_metric_value']:
                    is_best = True
                    stats['best_val_metric_value'] = val_metric_value
